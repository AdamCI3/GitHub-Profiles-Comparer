...
export class ProfilesComparerComponent {
  ...
  async getUserData(userIndex: number) {
    this.winner=""
    const gitHubName = userIndex === 1 ? this.gitHubName1ProvidedByUser : this.gitHubName2ProvidedByUser;
    const userDataFromService : any = await this.gitHubInfoService.getUserGitHubInfoByName(gitHubName);
  ...
  compare() {
    ...
    if (this.user1CreatedFromGitHubInfoService?.public_repos  > this.user2CreatedFromGitHubInfoService?.public_repos ) {
      user1Score++;
    } else if (this.user2CreatedFromGitHubInfoService?.public_repos > this.user1CreatedFromGitHubInfoService?.public_repos ) {
      user2Score++;
    }
    console.log(user1Score)
    console.log(user2Score)
    if (this.user1CreatedFromGitHubInfoService?.forks> this.user2CreatedFromGitHubInfoService?.forks ) {
      user1Score++;
    } else if (this.user2CreatedFromGitHubInfoService?.forks  > this.user1CreatedFromGitHubInfoService?.forks ) {
      user2Score++;
    }
    console.log(user1Score)
    console.log(user2Score)
    if (this.user1CreatedFromGitHubInfoService?.followers > this.user2CreatedFromGitHubInfoService?.followers ) {
      user1Score++;
    } else if (this.user2CreatedFromGitHubInfoService?.followers  > this.user1CreatedFromGitHubInfoService?.followers ) {
      user2Score++;
    }
    console.log(user1Score)
    console.log(user2Score)
    if (this.user1LanguageUsage.length > this.user2LanguageUsage.length) {
      user1Score++;
    } else if (this.user2LanguageUsage.length > this.user1LanguageUsage.length) {
      user2Score++;
    }
    console.log(user1Score)
    console.log(user2Score)
